...
namespace regOP {
	...
	uint32_t inner_bias;
	int shift_;
	std::vector<uint64_t> zero_patterns(64);
}

VHS::VHS(uint32_t mem, double _heap_ratio) : global_HLL(glb_HLL_size), SketchBase("VHS   ") {
	memory = mem;  //kB
	heap_size = memory * 1024 * _heap_ratio / sizeof(IdSpread);
	register_num = memory * 1024 * (1 - _heap_ratio) * 8 / 5;
	raw.resize(memory * 1024 * (1 - _heap_ratio) * 8 / 32 + 1);
	uint64_t allone = -1;
	uint64_t fiveone = 31;
...
void VHS::insert(uint32_t flowID, uint32_t elementID) {
	start_timing();
	// uint32_t hash_flow = str_hash32(flowID, HASH_SEED_1);
	uint32_t hash_eleID = str_hash32(elementID, HASH_SEED_1);
	uint32_t reg_pos = str_hash32(((uint64_t)flowID << 32ull) + (hash_eleID & (HLL_size - 1)), HASH_SEED_2) % register_num;
...
uint32_t VHS::get_spread(std::vector<uint8_t> virtual_HLL) {
...
uint32_t VHS::get_flow_cardinality(uint32_t flowID) {
	start_timing();
	std::vector<uint8_t> virtual_HLL(HLL_size);
	for (size_t i = 0; i < HLL_size; i++) {
		uint32_t reg_pos = str_hash32(((uint64_t)flowID << 32ull) + i, HASH_SEED_2) % register_num;
