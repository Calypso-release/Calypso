...
Bitmap_Arr::Bitmap_Arr(uint32_t memory_) : memory(memory_), bitmap_num(memory * 1024 * 8 / bitmap_size), raw(memory * 1024 * 8 / 32) {
	...
	//std::cout << "The number of LC(bitmap)s in layer 1: " << bitmap_num << std::endl;
}
...
bool Bitmap_Arr::check_flow_full(std::array<uint64_t, 2>& hash_flowid) {
	...
	std::bitset<bitmap_size> bm(bitmap_merged);
...
bool Bitmap_Arr::process_packet(std::array<uint64_t, 2>& hash_flowid, uint32_t l2_update_pos) {
...
bool Bitmap_Arr::process_packet(std::array<uint64_t, 2>& hash_flowid, std::array<uint64_t, 2>& hash_element) {
	if (check_flow_full(hash_flowid))
		return true;
...
int Bitmap_Arr::get_cardinality(uint64_t flowid, std::array<uint64_t, 2>& hash_flowid, uint32_t& bm_merged) {
	uint32_t esti_pos_1, esti_pos_2;
	esti_pos_1 = static_cast<uint32_t>(hash_flowid[0] >> 32) % bitmap_num;
	esti_pos_2 = static_cast<uint32_t>(hash_flowid[0]) % bitmap_num;
	...
	bm_merged = bm1 & bm2;
	std::bitset<bitmap_size> bm_bits(bm_merged);
...
void HyperLogLog::shared_param_Init() {
	...
	//std::cout << "HyperLogLog shared params initialized." << std::endl;
}
...
HyperLogLog HyperLogLog::query_merged_Estimator(HyperLogLog& HLL1, HyperLogLog& HLL2) {
	...
	for (size_t i = 0; i < register_num; i++) {
		uint32_t tmpval = (std::min)(HLL1.get_counter_val(i), HLL2.get_counter_val(i));
...
uint32_t HyperLogLog::generate_bitmap() {
	std::bitset<Bitmap_Arr::bitmap_size> bm2;
...
void MultiResBitmap::shared_param_Init() {
	...
	//std::cout << "MultiResBitmap shared params initialized.  " << "mrbitmap_size: " << mrbitmap_size << std::endl;
}
...
uint32_t MultiResBitmap::generate_bitmap() {
	std::bitset<Bitmap_Arr::bitmap_size> bm;
	std::vector<uint8_t> multi_layer_merged(bitmaps[0].size());
...
template<class EstimatorType>
void Layer2<EstimatorType>::process_packet(uint32_t flowid, std::array<uint64_t, 2>& hash_flowid, uint32_t hashres32) {
	uint32_t esti_pos_1, esti_pos_2;
	...
	int flow_digest = (std::max)(dg1, dg2);
	if (flow_digest > 0) {
...
template<class EstimatorType>
uint32_t Layer2<EstimatorType>::get_cardinality(uint32_t flowid, std::array<uint64_t, 2>& hash_flowid, uint32_t& layer2_bm) {
	uint32_t esti_pos_1, esti_pos_2;
	esti_pos_1 = static_cast<uint32_t>(hash_flowid[1] >> 32) % esti_num;
	esti_pos_2 = static_cast<uint32_t>(hash_flowid[1]) % esti_num;
...
template<class EstimatorType>
void Layer2<EstimatorType>::insert_hashtab(uint32_t flowid, int flow_digest, uint64_t hahsres64) {     // Power of Two
	...
	if (hash_table[table_pos1].flowid == 0 || hash_table[table_pos1].flowid == flowid) {
		hash_table[table_pos1].flowid = flowid;
		hash_table[table_pos1].digest = flow_digest;
		return;
	}
	else if (hash_table[table_pos2].flowid == 0 || hash_table[table_pos2].flowid == flowid) {
...
void Couper::report_superspreaders(std::vector<IdSpread>& superspreaders) {
	superspreaders.clear();
	std::set<uint32_t> checked_flows;
	for (size_t i = 0; i < layer2.tab_size; i++) {
		uint32_t tmp_flowid = layer2.hash_table[i].flowid;
...
std::map<uint32_t, uint32_t> Couper::get_heavy_hitter(double threshold)
{
	...
	for (size_t i = 0; i < layer2.tab_size; i++) {
		...
		if (checked_flows.find(tmp_flowid) != checked_flows.end())
			...
			if (esti_card >= threshold)
				ret[tmp_flowid] = esti_card;
		}
	}
	return ret;
}

void Couper::insert(uint32_t flowid, uint32_t element) {
	start_timing();
	std::array<uint64_t, 2> hash_flowid = str_hash128(flowid, HASH_SEED_1);
	std::array<uint64_t, 2> hash_element = str_hash128(((uint64_t)flowid << 32ull) + element, HASH_SEED_2);
...
int Couper::get_overlapping_bias(uint32_t bm_layer1, uint32_t bm_layer2) {
	...
	std::bitset<Bitmap_Arr::bitmap_size> bm_inter_bits(bm_intersection);
	uint32_t zero_inter = Bitmap_Arr::bitmap_size - bm_inter_bits.count();
	int bias = layer1.card_dict[zero_inter];
	return bias;
}

uint32_t Couper::get_flow_cardinality(uint32_t flowid) {
	start_timing();
	std::array<uint64_t, 2> hash_flowid = str_hash128(flowid, HASH_SEED_1);
	uint32_t layer1_bm;
	...
	if (cardinality_layer1 != BITMAP_FULL_FLAG)
		...
		// std::cout << overlapping_bias << std::endl;
...
std::array<double, 1 << HyperLogLog::register_size> HyperLogLog::exp_table;
