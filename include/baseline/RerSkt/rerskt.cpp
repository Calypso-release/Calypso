...
void HLL::record_element(uint32_t hash_elem, uint32_t unit_pos) {
	...
	HLL_registers[unit_pos] = (std::max)(lz_num, HLL_registers[unit_pos]);
}
void HLL::record_element(uint32_t hashres) {
	...
	HLL_registers[reg_pos] = (std::max)(lz_num, HLL_registers[reg_pos]);
}
int HLL::get_spread(std::array<uint8_t, HLL::register_num> virtual_HLL) {
	double inv_sum = 0;
...
uint32_t getbit(std::array<uint64_t, 2> hashres128, uint32_t pos) {
	uint32_t retbit;
	if (pos < 64)
...
RerSkt<Estimator>::RerSkt(uint32_t memory_, double _heap_ratio) : memory(memory_), SketchBase("rerSkt") {
	heap_size = memory_ * 1024 * _heap_ratio / sizeof(IdSpread);
	table_size = memory_ * 1024 * (1 - _heap_ratio) * 8 / 2 / Estimator::size;
	table1.resize(table_size), table2.resize(table_size);
...
template<class Estimator>
void RerSkt<Estimator>::insert(uint32_t flowid, uint32_t element) {
	start_timing();
	...
	std::array<uint64_t, 2> hashres_128 = str_hash128(((uint64_t)flowid << 32ull) + hash_batch, HASH_SEED_1);
	...
	{
		IdSpread p(flowid, inserted[flowid]);
		heap.erase(p);
		inserted.erase(flowid);
	}
	else if (heap.size() == heap_size)
	{
		IdSpread min_p = *heap.begin();
		if (min_p.spread < flowsrpead)
		{
			heap.erase(min_p);
			inserted.erase(min_p.flowID);
		}
	}
	if (heap.size() < heap_size)
	{
		inserted[flowid] = flowsrpead;
		IdSpread p(flowid, flowsrpead);
		heap.insert(p);
	}
	/*if (inserted.find(flowid) != inserted.end()) {
...
template<class Estimator>
uint32_t RerSkt<Estimator>::get_flow_cardinality(uint32_t flowid) {
	start_timing();
	...
	std::array<uint64_t, 2> hashres_128;
	for (size_t i = 0; i < primary_est.get_unit_num(); i++) {
		uint32_t modval = i % 128;
		if (modval == 0)
			hashres_128 = str_hash128(((uint64_t)flowid << 32ull) + i / 128, HASH_SEED_1);
